...
export default function Statement({ transactions, onBack }: StatementProps) {
  return (
    <div className="mt-8">
      <div role="region" aria-label="Account Statement">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {transactions.map((transaction, index) => (
                    <tr key={index}>
                      ...
                      <td
                        className="px-3 sm:px-6 py-4 whitespace-nowrap text-sm text-right text-gray-900"
                        aria-label={`Balance: $${transaction.balance.toFixed(
                          2
                        )}`}
                      >
